...
export async function generateMetadata({ params }: { params: { mainPath: string } }): Promise<Metadata> {
    const path = `/${params?.mainPath}`
...
const page = async ({ params }: any) => {
    const data = await serverFetch(GET_PAGE, { where: { path: { is: `/${params?.mainPath}` }, "status": "ACTIVE" } }, { cache: "no-store" });
